...
<div class="container-xxl flex-grow-1 container-p-y">
    <h4 class="py-3 mb-4"><span class="text-muted fw-light">الفنادق / {{ $tenant->name }} / الشركات /</span> تعديل {{ $company->name }}</h4>

    <div class="card mb-4">
        <div class="card-header d-flex justify-content-between align-items-center mb-3 border-bottom pb-3">
            <h5 class="mb-0 text-primary"><i class="bx mx-1 bx-edit"></i>تعديل البيانات الإدارية</h5>
        </div>
        <div class="card-body">
            @if($errors->any())
            ...
            <form action="{{ route('backend.tenants.companies.update', [$tenant, $company->id]) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="row g-3">
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="date" name="nazeel_account_expiry" class="form-control" value="{{ old('nazeel_account_expiry', $company->nazeel_account_expiry ? \Carbon\Carbon::parse($company->nazeel_account_expiry)->format('Y-m-d') : '') }}">
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="account_type" class="form-select" required>
                            <option value="أساسي" {{ old('account_type', $company->account_type) == 'أساسي' ? 'selected' : '' }}>أساسي</option>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" name="building_number" class="form-control" value="{{ old('building_number', $company->building_number) }}">
                    </div>
