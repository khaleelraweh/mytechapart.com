...
<div class="container-xxl flex-grow-1 container-p-y">
    <h4 class="py-3 mb-4"><span class="text-muted fw-light">الفنادق / {{ $tenant->name }} / الشركات والمرافق /</span> تعديل {{ $company->name }}</h4>

    <div class="card mb-4">
        <div class="card-header border-bottom d-flex justify-content-between">
            <h5 class="mb-0 text-primary"><i class="bx mx-1 bx-edit"></i>تعديل بيانات المنشأة</h5>
        </div>
        <div class="card-body pt-4">
            ...
            <form action="{{ route('backend.tenants.companies.update', [$tenant, $company->id]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <select name="parent_company_id" class="form-select">
                                    ...
                                    @foreach($companies as $comp)
                                        <option value="{{ $comp->id }}" {{ old('parent_company_id', $company->parent_company_id) == $comp->id ? 'selected' : '' }}>{{ $comp->facility_code }} - {{ $comp->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="status" class="form-select" required>
                                    <option value="نشط (أساسي)" {{ old('status', $company->status) == 'نشط (أساسي)' ? 'selected' : '' }}>نشط (أساسي)</option>
                                    <option value="نشط (Premium)" {{ old('status', $company->status) == 'نشط (Premium)' ? 'selected' : '' }}>نشط (Premium)</option>
                                    <option value="فرعي" {{ old('status', $company->status) == 'فرعي' ? 'selected' : '' }}>فرعي</option>
                                    <option value="موقوف" {{ old('status', $company->status) == 'موقوف' ? 'selected' : '' }}>موقوف</option>
                                </select>
                            </div>

                            <div class="col-md-6">
                                <label class="form-label">تاريخ انتهاء صلاحية حساب نزيل</label>
                                <input type="date" name="nazeel_account_expiry" class="form-control" value="{{ old('nazeel_account_expiry', $company->nazeel_account_expiry) }}">
                            </div>

                            <div class="col-md-6">
                                <label class="form-label">رمز المنشأة</label>
                                <input type="text" name="facility_code" class="form-control" value="{{ old('facility_code', $company->facility_code) }}">
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="number" name="max_units" class="form-control" required min="1" value="{{ old('max_units', $company->max_units) }}">
                            </div>

                            <div class="col-md-6">
                                <label class="form-label">اصدار الحساب <span class="text-danger">*</span></label>
                                <select name="account_type" class="form-select" required>
                                    <option value="اساسي" {{ old('account_type', $company->account_type) == 'اساسي' ? 'selected' : '' }}>اساسي</option>
                                    <option value="متقدم" {{ old('account_type', $company->account_type) == 'متقدم' ? 'selected' : '' }}>متقدم</option>
                                </select>
                            </div>
                ...
                <div class="mt-4 pt-4 border-top">
                    <button type="submit" class="btn btn-primary d-inline-flex align-items-center"><i class="bx bx-save me-1"></i> حفظ التعديلات</button>
